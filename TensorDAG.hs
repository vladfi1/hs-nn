...
tensorFloat =
  case (forallC :: Forall (Implies1 IntegralL (CompC Floating t))) of
    Forall (Dict :: Dict (Implies1 IntegralL (CompC Floating t) dims)) ->
      case ins of 
...
makeMV :: forall t n m. (Tensor t, IntegralL '[n, m]) => Node (t '[n, m]) -> Node (t '[m]) -> IO (Node (t '[n]))
